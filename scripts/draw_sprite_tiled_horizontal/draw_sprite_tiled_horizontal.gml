...
function draw_sprite_tiled_horizontal_part_ext(sprite, subimg, left, top, width, height, pos_x, pos_y, scale_x, scale_y, sprite_color, sprite_alpha, vertical = false){
	var Width, Height, Left, Right, Bottom;
	...
	if(!vertical)
	{
		for(var i = Left; i <= Right; i++)
		{
			draw_sprite_part_ext(sprite, subimg, left, top, width, height, pos_x mod Width+Width*i, pos_y,  scale_x, scale_y, sprite_color, sprite_alpha);
		}
	}
	else
	{
		for(var i = Left; i <= Right; i++)
		{
			for(var j = -1; j <= Bottom; j++)
			{
				draw_sprite_part_ext(sprite, subimg, left, top, width, height, pos_x mod Width+Width*i, pos_y mod Height+Height*j, scale_x, scale_y, sprite_color, sprite_alpha);
			}
		}		
	}
}
